...
(define (step-dependencies assignment-id)
  (lambda (step)
    (filter instructor-solution-dependency? (map (determine-dependency assignment-id (Step-id step)) (Step-reviews step)))))
...
(define (maybe-assign-reviewers assignment-id step uid)
  (let* ((reviews (filter student-submission? (Step-reviews step)))
         (step-id (Step-id step)))
    (maybe-assign-reviewers-helper assignment-id step-id uid reviews)))

(: maybe-assign-reviewers-helper (String String String (Listof Review) -> Void))
(define (maybe-assign-reviewers-helper assignment-id step-id reviewee reviews)
  (cond [(empty? reviews) (void)]
        [else (let* ((review (first reviews))
                     (review-id (Review-id review))
                     (review-amount (Review-amount review))
                     (pending (get-pending-reviews assignment-id step-id review-amount)))
                (begin
                  (handle-pending reviewee pending)
                  (maybe-assign-reviewers-helper assignment-id step-id reviewee (rest reviews))))]))

(: handle-pending (String (Listof (Vector String Integer)) -> Void))
(define (handle-pending reviewee pending)
  (cond [(empty? pending) (void)]
        [else (let* ((review-hash (vector-ref (first pending) 0))
                     (review (review:select-by-hash review-hash))
...
(provide hold-for-review-handler)
(: hold-for-review-handler AssignmentHandler)
(define hold-for-review-handler (AssignmentHandler default-next-action do-submit-step default-get-dependencies default-take-dependency "hold-for-review"))
